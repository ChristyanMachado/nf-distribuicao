-- Registro exato da atualização aplicada SOMENTE no QA szakgftippcqtuqwxsox.
-- Não executar em produção nem reaplicar; não contém migration 0014.
...
INSERT INTO drizzle.__drizzle_migrations(hash,created_at) VALUES (encode(extensions.digest($qa_source$-- Incremento opt-in. Aplicar não ativa a coordenação nem reabilita papéis antigos.
CREATE TABLE fiscal.worker_coordination (
  id boolean PRIMARY KEY DEFAULT true CHECK (id),
  enabled boolean NOT NULL DEFAULT false
);
...
REVOKE ALL ON FUNCTION fiscal.worker_start(text,uuid,text,integer),fiscal.worker_heartbeat(text,uuid,boolean,text),
 fiscal.worker_stop(text,uuid),fiscal.worker_admission(text,uuid),fiscal.worker_summary(text,uuid),
 fiscal.worker_recover_abandoned(text,uuid),fiscal.guard_worker_queue() FROM PUBLIC,anon,authenticated;
$qa_source$,'sha256'),'hex'),1789056000000);
...
INSERT INTO drizzle.__drizzle_migrations(hash,created_at) VALUES (encode(extensions.digest($qa_source$-- Custom SQL migration file, put your code below! --
-- Saldo físico de mercadoria devolvida, separado do histórico fiscal das
-- distribuições. O saldo é compartilhado por produto + mercado, mesmo se o
-- mercado puder ser atendido por mais de um emitente.
CREATE TABLE "fiscal"."trocas_mercado" (
...
-- o Worker não recebe acesso a saldos de troca.
REVOKE ALL ON TABLE "fiscal"."trocas_mercado" FROM PUBLIC, anon, authenticated;
$qa_source$,'sha256'),'hex'),1789185225385);
...
DECLARE r text;
BEGIN
 FOREACH r IN ARRAY ARRAY['nf_worker_local','nf_worker_vm'] LOOP
  IF EXISTS (SELECT FROM pg_roles WHERE rolname=r) THEN
   EXECUTE format('GRANT EXECUTE ON FUNCTION fiscal.reservar_impressao_roteiro(text,timestamptz), fiscal.iniciar_envio_roteiro(uuid,uuid), fiscal.finalizar_impressao_roteiro(uuid,uuid,text,text) TO %I',r);
  END IF;
 END LOOP;
END $qa_grants$;

INSERT INTO drizzle.__drizzle_migrations(hash,created_at) VALUES (encode(extensions.digest($qa_source$-- Fila privada de impressão. A existência do roteiro não implica impressão:
-- somente o Worker, após conferir todas as notas AUTORIZADA do lote, pode
-- reservar e encaminhar o PDF para a fila local da impressora.
CREATE TABLE fiscal.impressoes_roteiro (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
BEGIN
 ...
 END LOOP;
END $qa_grants$;
$qa_source$,'sha256'),'hex'),1789232400000);
...
INSERT INTO drizzle.__drizzle_migrations(hash,created_at) VALUES (encode(extensions.digest($qa_source$-- Uma UUID idempotente não pode aceitar conteúdo diferente em um reenvio.
-- A coluna permanece nula para lotes legados, que são deliberadamente tratados
-- como não reutilizáveis pelo Web para evitar uma duplicação silenciosa.
ALTER TABLE fiscal.lotes_distribuicao
  ADD COLUMN IF NOT EXISTS payload_hash text;
$qa_source$,'sha256'),'hex'),1789318800000);
...
INSERT INTO drizzle.__drizzle_migrations(hash,created_at) VALUES (encode(extensions.digest($qa_source$CREATE TABLE fiscal.trocas_lancamentos (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  chave_idempotencia uuid NOT NULL UNIQUE,
  cliente_id uuid NOT NULL REFERENCES fiscal.clientes(id),
  produto_id uuid NOT NULL REFERENCES fiscal.produtos(id),
...
ALTER TABLE fiscal.trocas_lancamentos ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON fiscal.trocas_lancamentos FROM PUBLIC, anon, authenticated;
$qa_source$,'sha256'),'hex'),1789318860000);
