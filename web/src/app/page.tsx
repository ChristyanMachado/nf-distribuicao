...
async function carregarResumoOperacional() {
  ...
  type ProntidaoBanco = {
    ...
    produtos: Array<{ codigoFiscal: string | null; regraFiscalId: string | null }>;
    relacoes: Array<{ clienteId: string }>;
  };

  // Um único round-trip traz somente os campos de prontidão. Além de não
  // projetar credenciais legadas, evita saturar o pequeno pool do Web com
    ...
    SELECT
      COALESCE((
        SELECT jsonb_agg(jsonb_build_object(
          ...
          'indicadorIe', c.indicador_ie,
          'destinatarioNome', c.destinatario_nome,
          'cep', c.cep,
          'numeroEndereco', c.numero_endereco
        ))
        FROM fiscal.clientes c WHERE c.ativo = true
      ), '[]'::jsonb) AS clientes,
      COALESCE((
        SELECT jsonb_agg(jsonb_build_object(
          'cnpj', e.cnpj,
          'inscricaoEstadual', e.inscricao_estadual,
          'credencialReferencia', e.credencial_referencia,
          'valorSelectNfpe', e.valor_select_nfpe
        ))
        FROM fiscal.emitentes e WHERE e.ativo = true
      ), '[]'::jsonb) AS emitentes,
      COALESCE((
        SELECT jsonb_agg(jsonb_build_object(
          'codigoFiscal', p.codigo_fiscal,
          'regraFiscalId', p.regra_fiscal_id
        ))
        FROM fiscal.produtos p WHERE p.ativo = true
      ), '[]'::jsonb) AS produtos,
      COALESCE((
        SELECT jsonb_agg(jsonb_build_object('clienteId', ce.cliente_id))
  ...
  const produtosAtivos = prontidaoBanco?.produtos ?? [];
  const relacoes = prontidaoBanco?.relacoes ?? [];

  const quantidadeEmitentesPorCliente = new Map<string, number>();
  for (const relacao of relacoes) {
    quantidadeEmitentesPorCliente.set(
      relacao.clienteId,
      (quantidadeEmitentesPorCliente.get(relacao.clienteId) ?? 0) + 1,
    );
  }
  const emitentesIncompletos = emitentesAtivos.filter(
    (emitente) => pendenciasEmitente(emitente).length > 0,
  ).length;
  const clientesIncompletos = clientesAtivos.filter(
    (cliente) => pendenciasCliente(
      cliente,
      quantidadeEmitentesPorCliente.get(cliente.id) ?? 0,
    ).length > 0,
  ).length;
  const produtosIncompletos = produtosAtivos.filter(
...
function PreparacaoFiscal({
  ...
  if (prontidao.pronta) {
    return (
      <p className="mt-3 rounded-[var(--radius-control)] bg-[var(--field-tint)] px-4 py-3 text-[13px] text-[var(--field-strong)]" role="status">
        Cadastros fiscais prontos para gerar novas tarefas.
      </p>
    );
  }
  ...
  const passos = [
    {
      href: "/emitentes",
      label: "Emitentes",
      pendentes: prontidao.emitentes === 0
        ? 1
        : prontidao.emitentesIncompletos,
    },
    {
      href: "/clientes",
      label: "Clientes",
      pendentes: prontidao.clientes === 0
        ? 1
        : prontidao.clientesIncompletos,
    },
    {
      href: "/produtos",
      label: "Produtos",
      pendentes: prontidao.produtos === 0
        ? 1
        : prontidao.produtosIncompletos,
    },
  ].filter((passo) => passo.pendentes > 0);

  return (
    <Card className="mt-3 border-[var(--wheat)] bg-[var(--wheat-tint)] p-4">
      <p className="font-semibold text-[var(--ink)]">Finalize a preparação fiscal</p>
      <p className="mt-1 text-[13px] leading-relaxed text-[var(--ink-soft)]">
        Corrija os cadastros abaixo antes de montar a próxima distribuição.
      </p>
      <div className="mt-3 flex flex-wrap gap-2">
        {passos.map((passo) => (
          <a
            key={passo.href}
            href={passo.href}
            className="tap-target inline-flex min-h-11 items-center rounded-[var(--radius-control)] border border-[var(--line-strong)] bg-[var(--paper-raised)] px-3 text-sm font-semibold text-[var(--field-strong)]"
          >
            {passo.label} · {passo.pendentes} pendente(s)
          </a>
        ))}
      </div>
    </Card>
