                ...
                return <div key={p.produtoId} className="rounded-[var(--radius-control)] border border-[var(--line)] p-3">
                  ...
                  <div className="mt-2 space-y-2">{linhas.map((linha) => {
                    ...
                    return <div key={chaveDestino(linha)} className={`rounded-[var(--radius-control)] border p-2.5 ${preenchido ? "border-[var(--field)]" : "border-[var(--line)]"}`}>
                      ...
                      {saldoInicialTroca > 0 || linha.trocaAberta ? <div className="mt-2 rounded border border-[var(--wheat)] bg-[var(--cream)] px-2 py-1.5"><p className="text-[11px] font-medium text-[var(--ink-soft)]">Trocas pendentes: <span className="font-mono-tab text-[var(--field-strong)]">{formatarQuantidade(saldoInicialTroca)} {produto.unidade}</span></p><div className="mt-1 grid grid-cols-[1fr_auto] items-center gap-2"><Label>Usar nesta entrega</Label><input type="number" inputMode="decimal" value={linha.quantidadeTroca} placeholder="0" aria-label={`Reposição de ${produto.descricao} para ${cliente.nome} — ${emitente.nome}`} min="0" max={maximoNestaLinha} onChange={(e) => atualizarLinha(p.produtoId, linha, "quantidadeTroca", e.target.value)} className="font-mono-tab h-8! min-h-0! w-16 text-right text-[12px]" /></div><p className={`mt-1 text-[11px] ${saldoRestanteTroca < -0.0005 ? "text-[var(--stamp)]" : "text-[var(--ink-soft)]"}`}>{saldoRestanteTroca < -0.0005 ? `Excede o pendente registrado em ${formatarQuantidade(Math.abs(saldoRestanteTroca))} ${produto.unidade}.` : `Continuará pendente: ${formatarQuantidade(saldoRestanteTroca)} ${produto.unidade}.`}</p></div> : <a href="/trocas" className="mt-2 inline-block text-[12px] text-[var(--ink-faint)] underline decoration-dotted underline-offset-2">Sem reposição pendente · registrar troca</a>}
