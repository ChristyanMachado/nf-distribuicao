...
export function montarHtmlRoteiroImpressao(
  ...
  const paradas = roteiro.map((parada, indice) => `
    ...
    <table><colgroup><col><col class="quantidade"><col class="quantidade"></colgroup><thead><tr><th>Produto</th><th>Total</th><th>Troca incluída</th></tr></thead><tbody>${parada.itens.map((item) => `<tr><td>${escapar(item.produtoDescricao)} <small>${escapar(item.unidade)}</small></td><td>${formatarQuantidade(item.quantidadeDistribuida)}</td><td class="troca">${item.quantidadeTroca > 0 ? formatarQuantidade(item.quantidadeTroca) : "—"}</td></tr>`).join("")}</tbody></table>
    <footer>Entregue ( ) &nbsp; Parcial ( ) &nbsp; Não entregue ( ) &nbsp; Observação: __________________</footer></section>`).join("");
  return `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>${escapar(titulo)}</title><style>@page{margin:12mm}*{box-sizing:border-box}body{font:12px Arial;color:#202b32;margin:0}h1{font-size:22px;margin:2px 0}p{margin:4px 0 18px;color:#52616b}.parada{border:1px solid #cad4d9;border-radius:7px;margin:0 0 12px;overflow:hidden}.parada header{break-after:avoid;display:flex;gap:10px;padding:10px;background:#edf4f2}.parada header>b{width:28px;height:28px;line-height:28px;text-align:center;border-radius:50%;background:#3e6c65;color:#fff}.parada h2{font-size:16px;margin:0}.parada small{color:#52616b}table{width:100%;border-collapse:collapse;table-layout:fixed}.quantidade{width:6rem}th,td{padding:8px 10px;text-align:left;border-top:1px solid #e2e7e9}tr{break-inside:avoid}th:nth-child(n+2),td:nth-child(n+2){text-align:right}th{font-size:10px;text-transform:uppercase;color:#52616b}.troca{color:#a43f32;font-weight:700}footer{padding:10px;border-top:1px dashed #cad4d9;color:#52616b;font-size:11px}</style></head><body><h1>Graalyst · roteiro de entrega</h1><p>${escapar(titulo)} · ${escapar(data)} · ${roteiro.length} parada(s)</p>${paradas}</body></html>`;
}
