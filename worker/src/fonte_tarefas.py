...
class FontePostgresTarefas:
    ...
    async def obter_politica_concorrencia(self) -> dict[str, int | str]:
        """Lê apenas a política do próprio executor, protegida por RLS."""
        identidade = executor_atual.get()
        if identidade is None or identidade.worker_id != self.worker_id:
            raise FonteTarefasErro("Identidade do executor não confirmada.")
        try:
            async with self._conexao() as conexao:
                linha = await conexao.fetchrow(
                    """SELECT requested_mode,manual_capacity,automatic_max,capacity_limit
                       FROM fiscal.workers WHERE worker_id=$1""",
                    self.worker_id,
                )
            if linha is None:
                raise FonteTarefasErro("Política do executor não encontrada.")
            modo = str(linha["requested_mode"])
            manual = int(linha["manual_capacity"])
            automatico = int(linha["automatic_max"])
            limite_admin = int(linha["capacity_limit"])
            if modo not in {"MANUAL", "AUTOMATICO"} or not all(
                1 <= valor <= 3 for valor in (manual, automatico, limite_admin)
            ):
                raise FonteTarefasErro("Política de concorrência inválida.")
            return {
                "mode": modo,
                "manual_capacity": manual,
                "automatic_max": automatico,
                "admin_limit": limite_admin,
            }
        except FonteTarefasErro:
            raise
        except Exception as exc:
            raise FonteTarefasErro("Não foi possível ler a política de concorrência.") from exc

    async def obter_fila_concorrencia(self) -> tuple[int, int]:
        """Conta tarefas reserváveis e credenciais distintas para decisão Auto."""
        try:
            async with self._conexao() as conexao:
                linha = await conexao.fetchrow(
                    """WITH candidatas AS (
                         SELECT t.id,t.payload_worker #>> '{tarefa,emitente,credencialReferencia}' AS credencial
                         FROM fiscal.tarefas t
                         WHERE t.status='PENDENTE' AND t.tentativas<3
                           AND t.lote_id IS NOT NULL AND t.contrato_versao=1
                           AND t.payload_worker IS NOT NULL AND t.payload_hash IS NOT NULL
                       ), ativas AS (
                         SELECT payload_worker #>> '{tarefa,emitente,credencialReferencia}' AS credencial
                         FROM fiscal.tarefas WHERE status IN ('PROCESSANDO','EMITINDO')
                           AND reserva_expira_em>clock_timestamp()
                         UNION
                         SELECT t.payload_worker #>> '{tarefa,emitente,credencialReferencia}'
                         FROM fiscal.cancelamentos_fiscais c
                         JOIN fiscal.notas n ON n.id=c.nota_id
                         JOIN fiscal.tarefas t ON t.id=n.tarefa_id
                         WHERE c.status='PROCESSANDO' AND c.reserva_expira_em>clock_timestamp()
                         UNION
                         SELECT t.payload_worker #>> '{tarefa,emitente,credencialReferencia}'
                         FROM fiscal.recuperacoes_documentos r
                         JOIN fiscal.notas n ON n.id=r.nota_id
                         JOIN fiscal.tarefas t ON t.id=n.tarefa_id
                         WHERE r.status='PROCESSANDO' AND r.reserva_expira_em>clock_timestamp()
                       ), disponiveis AS (
                         SELECT c.credencial FROM candidatas c
                         WHERE c.credencial IS NOT NULL AND c.credencial<>''
                           AND NOT EXISTS (SELECT 1 FROM ativas a WHERE a.credencial=c.credencial)
                       )
                       SELECT count(*)::integer AS tarefas,
                              count(DISTINCT credencial)::integer AS credenciais
                       FROM disponiveis"""
                )
            return int(linha["tarefas"]), int(linha["credenciais"])
        except Exception as exc:
            raise FonteTarefasErro("Não foi possível estimar a fila para concorrência automática.") from exc
